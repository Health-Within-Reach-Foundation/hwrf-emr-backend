...
const { clinicController, superadminController } = require('../../controllers');
const { patch } = require('./auth.route');

const router = express.Router();

router.route('/clinics').get(
  auth(),
...
router.route('/specialities').get(superadminController.getAllSpecialties);

router.route('/approve-clinic/:clinicId').patch(
  auth(),
  // roleAuthorization('superadmin'),
  validate(clinicValidation.approveClinic),
